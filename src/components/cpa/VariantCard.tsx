...
import type { VariantResult, SimulatedVariant } from "../../lib/cpa/types";
...
interface VariantCardProps {
  v: VariantResult | SimulatedVariant;
  affName: string;
...
const VariantCard: React.FC<VariantCardProps> = ({ v, affName, setCustomCpa }) => {
  ...
  return (
    ...
    >
      ...
      {/* Simulation section — only shown when setCustomCpa is provided (CPA Variavel context) */}
      {setCustomCpa && (() => {
        const sv = v as SimulatedVariant;
        return (
          <div className="vc-sim-box" style={{
            ...
            {sv.customCpa !== undefined && sv.customMargin !== undefined && (
              <div style={{ display: "flex", flexDirection: "column", gap: 4, marginTop: 8 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
                  <span style={{ color: "var(--text-2)" }}>Margem resultante:</span>
                  <span style={{
                    fontWeight: 600,
                    color: sv.customMargin >= 0 ? "var(--green-text)" : "var(--red)",
                  }}>
                    {fmtEur(sv.customMargin)}
                  </span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
                  <span style={{ color: "var(--text-2)" }}>Delta vs atual:</span>
                  <Delta value={sv.cpaDelta} unit="€" />
                </div>
              </div>
            )}
            ...
            {sv.simMaxCpa > 0 && (
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginTop: sv.customCpa !== undefined ? 8 : 0 }}>
                <span style={{ color: "var(--text-2)" }}>CPA max (margem alvo):</span>
                <span style={{ fontWeight: 600, color: sv.simCpaStatus === "reduce" ? "var(--red)" : "var(--green-text)" }}>
                  {fmtEur(sv.simMaxCpa)}
                </span>
              </div>
            )}
